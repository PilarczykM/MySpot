...
namespace MySpot.Api
{
    public static class UsersApi
    {
        ...
        public static WebApplication UseUsersApi(this WebApplication app)
        {
            app.MapGet(
                    "api/users/me",
                    async (HttpContext context, IQueryHandler<GetUser, UserDto> handler) =>
                    {
                        var userDto = await handler.HandleAsync(
                            new GetUser { UserId = Guid.Parse(context.User.Identity.Name) }
                        );
            ...
            app.MapGet(
                    ...
                    async (Guid userId, IQueryHandler<GetUser, UserDto> handler) =>
                    {
            ...
            app.MapPost(
                "api/users",
                async (SignUp command, ICommandHandler<SignUp> handler) =>
                {
